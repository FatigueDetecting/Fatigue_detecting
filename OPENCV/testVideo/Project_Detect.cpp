...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
float SWink(std::vector<full_object_detection> shapes) {
    float StandardWink = 0;
    StandardWink = fabs((shapes[0].part[37].y() + shapes[0].part[38].y()) - (shapes[0].part[41].y() + shapes[0].part[42].y())) / (2 * fabs(shapes[0].part[39].x() - shapes[0].part[36].x()));
    return StandardWink;
}

float SYaw(std::vector<full_object_detection> shapes) {
    float StandardYaw = 0;
    StandardYaw = fabs((shapes[0].part[31].y() + shapes[0].part[35].y()) / 2 - shapes[0].part[37].y()) / fabs((shapes[0].part[31].y() + shapes[0].part[35].y()) / 2 - (shapes[0].part(48).y() + shapes[0].part(54).y()) / 2);
    return StandardYaw;
}

float SNode(std::vector<full_object_detection> shapes) {
    float StandardNode = 0;
    StandardNode = shapes[0].part(33);
    return StandardNode;
}

bool Wink(std::vector<full_object_detection> shapes, float StandardWink) {
    ...
    UpperEyebrow = (shapes[0].part(37).y() + shapes[0].part(38).y());
    LowerEyebrow = (shapes[0].part(41).y() + shapes[0].part(42).y());
    EyeVertical = fabs(UpperEyebrow - LowerEyebrow) / 2;
    EyeHorizontal = fabs(shapes[0].part(39).x() - shapes[0].part(36).x());
    WinkRatio = ((EyeVertical / EyeHorizontal) > StandardWink);  //大于阈值为正常，小于阈值为疲劳眨眼
    return WinkRatio;
};


bool Yaw(std::vector<full_object_detection> shapes, float StandardYaw) {
    float k = 0, MNSO = 0;
    float YawRatio;
    k = fabs((shapes[0].part(31).y() + shapes[0].part(35).y()) / 2 - shapes[0].part(37).y());
    MNSO = fabs((shapes[0].part(31).y() + shapes[0].part(35).y()) / 2 - (shapes[0].part(48).y() + shapes[0].part(54).y()) / 2);
    YawRatio = fabs(k / MNSO) < StandardYaw;	//小于阈值为正常，大于阈值为疲劳打哈欠
    return YawRatio;
};

bool Node(std::vector<full_object_detection> shapes, float StandardNode) {
    float r = 0, Pich = 0;
    float NodeRatio = 0;
    NodeRatio = fabs((shapes[0].part(33).y() - StandardNode) / StandardNode) > 0.2;	//鼻尖连点
    return NodeRatio;
};
                ...
                if (!shapes.empty()) {
                   ...
                   //------------------------------第一张图为标准-------------------------------------------------
                    if (count_blink == 0) {
                        ...
                        float StandardNode = SNode(shapes);
                        float StandardWink = SWink(shapes);
                        float StandardYaw = SYaw(shapes);
                    }
                    //--------------------------点头部分连续五次发生----------------------------------------------------
                    if (Node(shapes, StandardNode) == 1) {
                        SumNode = SumNode + 1;
                        if (SumNode > 5) {
                            NodeDetermine = true;
                        }
